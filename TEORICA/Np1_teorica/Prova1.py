...
class MongoDBCollection:
    mongo_uri = "mongodb+srv://antonio:1234@np1.fz3nv1d.mongodb.net/?retryWrites=true&w=majority&appName=Np1"
    collection = None
    ...
    def get_collection():
        if not MongoDBCollection.collection:
            ...
            database = client.get_database('rpgks')
            MongoDBCollection.collection = database.get_collection('item')
...
class ItemDAO:
    ...
    def add_item(self, item : Item):
        """Questão 1 - Inserir item no MongoDB"""
        self.mongo_collection.insert_one(item.to_dict())

    def get_available_itens(self, capacity, coins):
        """Questão 1 - Buscar itens disponíveis por capacidade e moedas"""
...
class PageDAO:
    ...
    def add_page(self, page : Page):
        """Questão 2 - Inserir página no Neo4j com relacionamentos"""
        with self.neo4j_driver.session() as session:
            ...
            for characteristic in page.characteristics:
                session.run("""
                    ...
                    MERGE (c:Characteristic {name: $characteristic})
                    MERGE (p)-[:REQUIRES_ACCESS]->(c)
...
class CharacterDAO:
    ...
    def add_character(self, character: Character):
        """Questão 2 - Inserir personagem no Neo4j"""
        with self.neo4j_driver.session() as session:
            ...
            for profession in character.profession:
                session.run("""
                    ...
                    MERGE (p:Characteristic {name: $profession})
                    MERGE (c)-[:HAS_CHARACTERISTIC]->(p)
            ...
            for culture in character.culture:
                session.run("""
                    MATCH (c:Character {name: $name})
                    MERGE (cu:Characteristic {name: $culture})
                    MERGE (c)-[:HAS_CHARACTERISTIC]->(cu)
                """, name=character.name, culture=culture)

    def get_knowledge(self, character_name):
        """Questão 2 - Buscar páginas que um personagem conhece"""
        with self.neo4j_driver.session() as session:
            result = session.run("""
                MATCH (c:Character {name: $character_name})-[:HAS_CHARACTERISTIC]->(char)<-[:REQUIRES_ACCESS]-(p:Page)
